...
int run_add(const char *argv[] , int argc);
...
int multi_add_operation(const char* arc[] , int);
void show_staged_or_not(int);
void search_stagedfiles(int n);
int wildcard_add_operation(const char *);
int run_reset(const char *argv[] , int argc);
int normal_reset_operation(const char *file_path , int mode);
...
int multi_reset_operation(const char *argv[] , int argc);
int undo_reset_operation();
int check_status_M(char *cwd);
void check_status_A(char * cwd);
void commit_operation_without_space(const char *argv[]);
void commit_operation_with_space(const char *argv[] , int args);
int run_status();
void delete_files(char *folder_b);
void copy_dir(char *srcpath, char *destpath);
void copy_files(char *srcpath, char *destpath);
int run_remove(const char *argv[] , int argc);
int run_replace(const char *argv[] , int argc);
int run_set(const char *argv[] , int argc);
void commit_operation_with_shortcut(const char *argv[] , int argc);
void commit_operation_with_space(const char *argv[] , int args);
void commit_operation_with_space(const char *argv[] , int args);
void commit_operation_without_space(const char *argv[]);
int run_commit(const char *agrv[] , int argc);
void print_commit_log(struct commit commit_info);
void normal_log();
int run_log(const char *argv[] , int argc);
void since_before_log(const char *argv[] , int mode);
void branch_author_log(char *branch_author , int mode);
void n_log(char * num);
int run_branch(const char *argv[] , int argc);
void finding_neogit_path();
int is_staging_area_empty();

void checkout_commit(char *commit_name);
int run_checkout(const char *argv[] , int argc);
void checkout_branch(const char *bran);
int run_config(const char *argv[], int argc);
...
int multi_search(const char*argv[] , int argc);
...
int redo_addition(const char *argv[] , int argc);
int run_tag(const char *argv[],int  argc);
...
int main(int argc, char const *argv[]){
    ...
    else if(strcmp(argv[1] , "pre-commit") == 0) return run_precommit(argv , argc);
    else if(strcmp(argv[1] , "tree") == 0) return run_tree(argv , argc);
} 
...
int run_config(const char *argv[] , int argc){
    ...
    if(strcmp("-global" , argv[2]) == 0){
        ...
        if(strcmp("user.name" , argv[2]) == 0){
            char command[max];
            finding_neogit_path();
            snprintf(command ,max , "%s\\.neogit\\local_username.txt" , neogit_folder_location);
        ...
        if(strcmp("user.email" , argv[2]) == 0){
            char command[max];
            finding_neogit_path();
            snprintf(command ,max , "%s\\.neogit\\local_useremail.txt" , neogit_folder_location);
            FILE *file = fopen(command , "w");
            fprintf(file , "%s" , argv[3]);
            fclose(file);
        }
...
int run_init(){
    ...
    if(check_exist_neogit_folder(cwd) == 0){
        ...
        mkdir("hooks");
        chdir("hooks");
        system("echo static_error_check.txt >> all_hooks.txt");
        system("echo eof_blank_space.txt >> all_hooks.txt");
        system("echo balance_braces.txt >> all_hooks.txt");
        system("echo todo_check.txt >> all_hooks.txt");
        ...
        system("echo 0 > static_error_check.txt");
...
int run_add(const char *argv[] , int argc){
    ...
    char newpath[max];
    snprintf(newpath , max , "%s\\.neogit\\head_or_not.txt" , neogit_folder_location);
    
    FILE * file = fopen(newpath , "r");
    char word[50];
    fscanf(file , "%s" , word);
    if(strcmp(word , "HEAD")!= 0){
        printf("YOU SHOULD BE ON HEAD TO ADD\n");
        return 0;
    }
...
int multi_add_operation(const char *argv[] , int argc){
...
int wildcard_add_operation(const char *file_name){
    // printf("resed");
...
int redo_addition(const char *argv[] , int argc){
    ...
    while(fgets(line, max ,redo_file)){
        int n = strlen(line);
        if(line[n-1] == '\n') line[n-1] = '\0';
        normal_add_operation(line,500);
        fgets(line , max , redo_file);
    }
    fclose(redo_file);
    char command[max];
    snprintf(command , max ,"echo. > %s" , path);
    system(command);
}
...
int undo_reset_operation(){
    ...
    int n = strlen(third_line);
    if(third_line[n-1] == '\n') third_line[n-1] = '\0';
    normal_reset_operation(third_line , 1);
    return 0;
}
...
void commit_operation_with_shortcut(const char *argv[] , int argc){
    char path1[max];
    snprintf(path1 ,max , "%s\\.neogit\\last_commit.txt" , neogit_folder_location);
...
void branch_author_log(char *branch_author , int mode){
    ...
    if(mode == 2){
        for(int i = num_of_all_commits - 1 ; i>=0 ; i--){
            if(strstr(all_commits[i].message , branch_author) != NULL){
                print_commit_log(all_commits[i]);
            }
        }
    }
...
int run_branch(const char *argv[], int argc) {
    ...
    if(argc == 3){
        ...
        // printf("%s\n" , command);
        char ww[max];
        snprintf(ww,max , "%s\\.neogit\\branch_name.txt" , neogit_folder_location);
...
void checkout_branch(const char *bran){
    ...
    char line1[max];
    int num = 0;
        fgets(line1 , max , staged_path);
    while (fgets(line1 , max , staged_path)) 
    {
        int n = strlen(line);
        if(line1[n-1] == '\n') line1[n-1] = '\0';
        strcpy(all_staged_path[num] , line1);
        num++;
        fgets(line1 , max , staged_path);
    }
    fclose(staged_path);
    ...
    for(int i = 0 ; i < num ; i++){
        ...
        int n = strlen(all_staged_path[i]);
        if(all_staged_path[i][n-1] == '\n') all_staged_path[i][n-1] = '\0';
        ...
        system(com);
        char command2[max];
        snprintf(command2 , max , "copy /Y %s\\.neogit\\commits\\%s\\%d\\%s %s" , neogit_folder_location , all_commits[place].branch , all_commits[place].id, name , copy);
        printf("%s\n" , command2);
        system(command2);
        // system("Y");
    }
    ...
    char ppaa[max];
    snprintf(ppaa , max , "%s\\.neogit\\branch_name.txt" , neogit_folder_location);
    FILE *fop = fopen(ppaa , "w");
    fprintf(fopa ,"%s", bran);
    fclose(fop);
...
int is_staging_area_empty(){
    char path[max];
    snprintf(path ,max , "%s\\.neogit\\staging_area\\staged_files" , neogit_folder_location);
...
void replace_tag(char *message , char* name){
    ...
    while (fgets(line , max ,fp))
    {
        ...
        if(strcmp(line , name) == 0){
            strcpy(all_lines[num] , name);
            num++;
            fgets(line, max ,fp);
            n = strlen(line);
            if(line[n-1] == '\n') line[n-1] = '\0';
            strcpy(all_lines[num] , line);
            num++;
            fgets(line , max ,fp);
            strcpy(all_lines[num] , message);
            num++;
            fgets(line, max ,fp);
            n = strlen(line);
            if(line[n-1] == '\n') line[n-1] = '\0';
...
int run_precommit(char *argv[] , int argc){
    ...
    if(argc == 2){
        ...
        char command[max];
        snprintf(command, max , "%s\\.neogit\\staging_area" , neogit_folder_location );
        chdir(command);
        char command2[max];
        ...
        while(fgets(line , max , file)) {
            ...
            char line2[max];
            printf("%s:\n" , line);
            while(fgets(line2 , max , file)){
                char path2[max];
                int mm = strlen(line2);
                if(line2[mm-1] == '\n') line2[mm-1] = '\0';
                snprintf(path2 , max ,"%s\\.neogit\\hooks\\%s" , neogit_folder_location , line2);
                // printf("%s\n" , path2);
                FILE *fp = fopen(path2 , "r");
                int s;
                fscanf(fp , "%d" ,&s);
                // printf("line2 : %s , n ; %d\n" , line2 , n);
                if(s == 1){
                    if(strcmp(line2 , "eof_blank_space.txt") == 0){
                        // printf("salam");
                    ...
                    if(strcmp(line2 , "static_error_check.txt ") == 0){
                        if(checkStaticErrors(path) == 1){
                            printf("static_error_check : faild\n");
                        }
                        else{
                            printf("static_error_check : passed\n");
                        }
                    }
                    // if(strcmp(line2 , "time_limit.txt") == 0){
                    //     if(isFileMoreThanTenMinutes(path) == 0){
                    //         printf("time_limit : failed\n");
                    //     }
                    //     else printf("time_limit : passed\n");
                    // }
                    if(strcmp(line2 , "balance_braces.txt ") == 0){
                        if(balance_braces(path) == 0){
                            printf("balance_braces : failed\n");
                        }
                        else printf("balance_braces : passed\n");
                    }
                }    
            }
        }
...
const char *extension = strrchr(path, '.'); 
    ...
    lowercase[i] = '\0'; 
    if (strcmp(lowercase, ".c") == 0 || strcmp(lowercase, ".cpp") == 0 || strcmp(lowercase, ".txt") == 0) { 
        FILE *file = fopen(path, "r"); 
        if (file == NULL) { 
...
int more_than_20000(const char *filename) { 
...
int checkStaticErrors(const char *filename) {
    char command[100];
    snprintf(command, sizeof(command), "gcc -fsyntax-only %s", filename);

    int result = system(command);

    if (result == 0) {
        printf("No static errors found.\n");
        return 0;
    } else {
        printf("Static errors found in file: %s\n", filename);
        return 1;
    }
}
...
int checkBracketsMatching(const char *text) {
    Stack stack;
    ...
    for (int i = 0; text[i] != '\0'; i++) {
        if (text[i] == '(' || text[i] == '{' || text[i] == '[') {
            ...
            if (stack.top == -1 || !isMatchingPair(stack.data[stack.top], text[i])) {
    ...
    return stack.top == -1 ? 1 : 0; // If stack is empty, brackets are matching
}
int balance_braces(char *path){
    char line[max];
    FILE *file = fopen(path ,"r");
    int balance_or_not = 1;
    while(fgets(line , max ,file)) {
        if(checkBracketsMatching(line) == 0){
            return 0;
        }
    }
    return 1;
...
int run_tree(char *argv[] , int argc){
    finding_neogit_path();
    char path[max];
    snprintf(path ,max , "%s\\.neogit\\commits" , neogit_folder_location);
    chdir(path);
    char pathcopy[max];
    ...
    // printf("%s\n" , path);
        ...
        if(S_ISDIR(file_stat.st_mode) && strcmp("." , entry->d_name) != 0 && strcmp(".." , entry->d_name) != 0){
            showtree(path , entry->d_name);
        }
        strcpy(path , pathcopy);
    }
    return 0;
}
void showtree(char *path , char * branch){
    chdir(path);
    DIR * dir = opendir(".");
    // struct dirent *dir = opendir(".");
    int all_commits_num[100];
    for(int w = 0 ; w < 100 ; w++){
        all_commits_num[w] = -1;
    }
    struct dirent *entry;
    ...
    while((entry = readdir(dir) ) != NULL){
        ...
        all_commits_num[num] = atol(entry->d_name);
        num++;
        }
    }
    printf("%s:\n" , branch);
